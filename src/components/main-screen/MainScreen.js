...
class MainScreen extends Component {
  ...
  serverAddr() {
    if('true' === process.env.REACT_APP_IS_DEV_ENV) {
      ...
      return location.protocol.replace('http', 'ws') + '//' + host + (location.port ? ':'+5000 : '');
    } else {
  ...
  gameContent() {
    return (<Col s={12} m={this.state.state === 'created'? 12: 8}>
  ...
  playerList() {
    return (<Col s={12} m={4} style={{borderLeft: '1px solid #efefef', height: '100%', boxShadow: '-2px 0px 12px -7px rgba(0,0,0,0.51)'}}>
    ...
    return (
      <Row className="App">
        ...
        <Row className="App-content container">
          {!this.state.state && <Row className="row">
